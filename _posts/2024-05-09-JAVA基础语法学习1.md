...
创建项目名称后，右击项目的 src 目录，在弹出菜单中选择 New→Package，然后输入包名，右击包名选择 New→Java Class。
...
一个类中可以定义名称相同、参数不同的方法。构造方法和普通方法都能重载。
