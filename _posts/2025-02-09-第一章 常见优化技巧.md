...
related: true
toc: true
description: "常见优化技巧包含4个部分：双指针法、空间换时间、单调栈、单调队列。"
---

常见优化技巧包含 4 个部分：双指针法，空间换时间，单调栈，单调队列。
...
用空间换取时间，可以使用一些技巧提升处理数据的效率。如利用数组下标实现 $$O(1)$$ 的速度获取数据，利用 `map[x]` 查询的时间复杂度是 $$O(log_2n)$$（n 是 map 中元素个数）。

运用桶排序的思想，是最常用的空间换取时间。

对于一个直接难以处理的式子，我们可以将式子拆开进行处理，再将一些部分进行合并计算就可以获得更低复杂度的算法。

习题1：[P7072 直播获奖](https://www.luogu.com.cn/problem/P7072)

习题2：[P2671 求和](https://www.luogu.com.cn/problem/P2671)

习题3：[P4147 玉蟾宫](https://www.luogu.com.cn/problem/P4147)
...
习题1：[P2866 Bad Hair Day S](https://www.luogu.com.cn/problem/P2866)
...
习题2：[P1950 长方形](https://www.luogu.com.cn/problem/P1950)
